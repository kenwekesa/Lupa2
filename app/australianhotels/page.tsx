...
import getAustralianHouseLisings from "../acts/getAustalianHousesListings";
...
const DestinationPage = async ({ searchParams, tourParams }: HotelPageProps) => {
  ...
  const listings = await getAustralianHouseLisings({ ...searchParams, hotel: "hotel", continent: "australia"});
  ...
  return (
    ...
    <div className="all-destinations-main flex flex-col items-center justify-center text-lg font-bold">
        <h1 className="color-h1-destinations-main">Australian <span className="color-span-green">Hotels</span></h1>
      ...
      <Container>
        ...
        {/* <h1 className="main-header-black w-full text-center pt-9 pb-0">AUSTRALIAN <span className="main-header-gradient py-1">HOTELS</span></h1> */}
        <p className="text-md text-neutral-600 leading-8 pt-6 pb-0 text-md w-full text-justify">Embark on unparalleled Australian hotel listings with our prime selection. Curated by experts, these accommodations bring sought-after destinations to your doorstep. From coastal resorts to outback retreats, immerse yourself in luxurious stays filled with natural beauty, wildlife, and cultural richness. Enhance your experience with personalized tours, redefining your travel expectations.</p>
        </div>
      </Container>
      <Container>
          <div className="pb-0">
      ...
      <div className="grid-cols-page-s pt-6 pb-4 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-8">
        ...
        <div className="w-full text-center pt-8">
          <Link className="outline-main-btn px-4 hover:bg-slate-400 hover:text-green-400 hover:shadow-md" href="/allaustralianhotels">View Australian Hotels</Link>
        </div>
          </Container>
