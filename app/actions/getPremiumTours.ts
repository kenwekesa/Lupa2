...
// export default async function getPremiumTours(
//     params: IToursParams
// ) {
//     try {
//         const {
...
//         if (deal && deal.toLowerCase() === 'premium') {
...
export interface IToursParams {
    ...
    bathroomCount?: number;
    startDate?: string;
    deal?: string; // Corrected to lowercase 'string'
    endDate?: string;
    locationValue?: string;
    category?: string;
}
...
export default async function getPremiumTours(
    ...
    try {
       ...
       const { userId: userIdParam, ...restParams } = params || {};
        ...
        // Add condition for deal to be "premium"
        if (deal && deal.toLowerCase() === 'premium') {
            query.deal = deal.toLowerCase();
        }
