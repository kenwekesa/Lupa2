...
import getCanadaTours from "@/app/acts/getCanadaTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getCanadaTours({ ...tourParams, country: "canada" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Canadian<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Canadian adventure with our expertly crafted tours. Explore iconic landscapes from the majestic Rockies to the rugged coastlines of Nova Scotia. Encounter rich history and culture in Toronto&apos;s bustling streets and charming Quebec City. Immerse yourself in the culinary delights of Vancouver and discover the natural wonders of Banff National Park. Let us guide you through Canada&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
