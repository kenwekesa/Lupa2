...
import getSaTours from "@/app/acts/getSaTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getSaTours({ ...tourParams, country: "saudiarabia" });
    ...
    const totalPages = Math.ceil(visibleTours.length / PAGE_SIZE);
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Saudi Arabian<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Saudi Arabian adventure with our expertly crafted tours. Explore iconic landscapes from the majestic Red Sea coast to the bustling streets of Riyadh. Encounter rich history and culture in Jeddah&lsquo;s vibrant neighborhoods and charming historical sites. Immerse yourself in the culinary delights of Saudi Arabian cuisine and discover the ancient wonders of Al-Ula. Let us guide you through Saudi Arabia&lsquo;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
