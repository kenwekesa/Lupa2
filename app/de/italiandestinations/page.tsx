...
import getItalyTours from "@/app/acts/getItalyTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getItalyTours({ ...tourParams, country: "italy" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Italian<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Italian adventure with our expertly crafted tours. Explore iconic landscapes from the rolling hills of Tuscany to the rugged cliffs of the Amalfi Coast. Encounter rich history and culture in Rome&apos;s ancient ruins and charming villages. Immerse yourself in the culinary delights of Florence and discover the artistic wonders of Venice&apos;s canals. Let us guide you through Italy&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
