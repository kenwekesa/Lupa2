...
import getAustralianTours from "@/app/acts/getAustralianTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getAustralianTours({ ...tourParams, country: "australia" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Australian<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Australian adventure with our expertly crafted tours. Explore iconic landscapes from the majestic Great Barrier Reef to the bustling streets of Sydney. Encounter rich history and culture in Melbourne&apos;s vibrant laneways and charming coastal towns. Immerse yourself in the culinary delights of Australian cuisine and discover the natural wonders of the Outback. Let us guide you through Australia&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
