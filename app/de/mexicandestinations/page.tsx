...
import getMexicoTours from "@/app/acts/getMexicoTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getMexicoTours({ ...tourParams, country: "mexico" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Mexican<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Mexican adventure with our expertly crafted tours. Explore iconic landscapes from the ancient ruins of Chichen Itza to the pristine beaches of Cancun. Encounter rich history and culture in Mexico City&apos;s bustling streets and charming colonial towns. Immerse yourself in the culinary delights of Oaxaca and discover the natural wonders of the Yucatan Peninsula. Let us guide you through Mexico&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
