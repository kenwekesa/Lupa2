...
import getUkTours from "@/app/acts/getUKTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getUkTours({ ...tourParams, country: "uk" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime UK<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary UK adventure with our expertly crafted tours. Explore iconic landscapes from the lush greenery of the Lake District to the dramatic cliffs of the Jurassic Coast. Encounter rich history and culture in London&lsquo;s bustling streets and charming villages. Immerse yourself in the culinary delights of Edinburgh and discover the architectural wonders of Stonehenge. Let us guide you through the UK&lsquo;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
