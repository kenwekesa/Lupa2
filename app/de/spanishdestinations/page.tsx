...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getSpainTours({ ...tourParams, country: "spain" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Spanish<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Spanish adventure with our expertly crafted tours. Explore iconic landscapes from the sun-kissed beaches of the Costa del Sol to the majestic peaks of the Pyrenees. Encounter rich history and culture in Madrid&lsquo;s bustling streets and charming villages. Immerse yourself in the culinary delights of Barcelona and discover the architectural wonders of Gaudi&lsquo;s masterpieces. Let us guide you through Spain&lsquo;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
