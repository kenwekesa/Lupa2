...
import getUaeTours from "@/app/acts/getUaeTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getUaeTours({ ...tourParams, country: "uae" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime UAE<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary UAE adventure with our expertly crafted tours. Explore iconic landscapes from the dazzling skyscrapers of Dubai to the historic streets of Abu Dhabi. Encounter rich culture and heritage in the vibrant souks of Sharjah and charming traditional villages. Immerse yourself in the culinary delights of Emirati cuisine and discover the modern wonders of the Palm Jumeirah. Let us guide you through the UAE&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
