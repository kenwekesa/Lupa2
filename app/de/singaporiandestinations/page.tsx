...
import getSingaporeTours from "@/app/acts/getSingaporeTours";
...
export const metadata: Metadata =  {
  title: "Singaporian Destinations",
}
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getSingaporeTours({ ...tourParams, country: "singapore" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Singaporian<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Singaporean adventure with our expertly crafted tours. Explore iconic landscapes from the vibrant Gardens by the Bay to the bustling streets of Chinatown. Encounter rich history and culture in Singapore&apos;s diverse neighborhoods and charming heritage sites. Immerse yourself in the culinary delights of hawker centers and discover the modern wonders of Marina Bay Sands. Let us guide you through Singapore&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
