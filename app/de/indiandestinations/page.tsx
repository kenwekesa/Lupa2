...
import getIndiaTours from "@/app/acts/getIndiaTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getIndiaTours({ ...tourParams, country: "india" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Indiana<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Indian adventure with our expertly crafted tours. Explore iconic landscapes from the majestic Taj Mahal to the bustling streets of Mumbai. Encounter rich history and culture in Delhi&apos;s ancient monuments and charming traditional markets. Immerse yourself in the culinary delights of Indian cuisine and discover the spiritual wonders of Varanasi. Let us guide you through India&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
