...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getCountriesTours({ ...tourParams, country: "uganda" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime Ugandan<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary Ugandan adventure with our expertly crafted tours. Explore iconic landscapes from Bwindi Impenetrable Forest to the Rwenzori Mountains. Encounter majestic wildlife and experience the thrill of safari in Queen Elizabeth National Park. Immerse yourself in the cultural richness of Kampala&apos;s bustling markets and discover the ancient wonders of the Kasubi Tombs. Let us guide you through Uganda&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
