...
import getSouthAfricaTours from "@/app/acts/getSouthAfricaTours";
...
export default function AllDestinationsPage({ tourParams }: IParams) {
  ...
  const getToursAndRender = async () => {
    const tours = await getSouthAfricaTours({ ...tourParams, country: "southafrica" });
      ...
      <div>
        <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
          <h1 className="alldestinations-white-main">
            Prime South African<span className="color-span-green"> Tour Destinations</span>
          </h1>
        </div>
        <Container>
          <div className="flex flex-col gap-1 pt-9">
          <p className="text-md text-neutral-600 leading-8 text-md w-full text-justify">Embark on an extraordinary South African adventure with our expertly crafted tours. Explore iconic landscapes from Kruger National Park to Table Mountain. Encounter majestic wildlife and experience the thrill of safari. Immerse yourself in Cape Town&apos;s vibrant culture and discover the rich history of Robben Island. Let us guide you through South Africa&apos;s diverse wonders for an unforgettable journey.</p>
          </div>
        </Container>
        <Container>
          <div className="py-6">
