...
const DestinationPage = async ({ searchParams, tourParams }: HotelPageProps) => {
  ...
  const listings = await getAmericanHouseListings({ ...searchParams, house: "house", continent: "america"});
  const tours = await getTours(tourParams);
  const filteredTours = tours.filter(tour => tour.tourists.length < tour.guestCount).slice(0, 4);
  const filteredTourss = tours.filter(tour => tour.tourists.length < tour.guestCount).slice(0, 10);
  // const isEmpty = true;
  ...
  return (
    ...
    <div className="all-destinations-main flex flex-col items-center justify-center text-lg font-bold">
      ...
      <Container>
        ...
        <p className="text-md text-neutral-600 leading-8 pt-6 pb-0 text-md w-full text-justify">Embark on unparalleled Americas house listings with our prime selection. Curated by experts, these properties bring sought-after destinations to your doorstep. From mountain retreats to beachfront villas, immerse yourself in luxurious stays filled with natural beauty, wildlife, and cultural richness. Enhance your experience with personalized tours, redefining your travel expectations.</p>
        </div>
      </Container>
      <Container>
          <div className="pb-0">
      ...
      <div className="grid-cols-page-s pt-6 pb-4 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-8">
        ...
        <div className="w-full text-center pt-8">
          <Link className="outline-main-btn px-4 hover:bg-slate-400 hover:text-green-400 hover:shadow-md" href="/allamericanhouseleasing">View american house leasing</Link>
        </div>
        </Container>
