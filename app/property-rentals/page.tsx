...
export default function PrpertySales({ tourParams }: IParams) {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        const response = await axios.get('/api/property', { params: { ...tourParams, type: 'rental' } });
    ...
    <div>
      <div className="alldestinations-main flex flex-col items-center justify-center text-lg font-bold">
        <h1 className="alldestinations-white-main">
          <span className="text-white">Our {deal || "luxurious"} stays</span>
        </h1>
      </div>
      <Container>
