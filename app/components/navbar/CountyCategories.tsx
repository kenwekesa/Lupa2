...
export const counties = [
    {
        ...
        description: "These are stays!"
    },
    {
        label: 'Property Sales',
        icon: GiTreehouse,
        description: "These are property sales!"
    },
    {
        label: 'Property rentals',
        icon: GiHouse,
        description: "These are property rentals!"
    },
    {
        label: 'Land sales',
        icon: MdOutlineLandslide,
        description: "These are land sales!"
    }
]
...
const CountyCategories = () => {
    ...
    const handleNextClick = () => {
        ...
        setStartIndex(Math.min(newStartIndex, counties.length - 9));
    };
  ...
  return (
    <Container>
            <div className="pt-4 flex flex-row items-center justify-between overflow-x-auto relative">
                ...
                {counties.slice(startIndex, startIndex + 9).map((item) => (
                    <CategoryBox
                        key={item.label}
                        label={item.label}
                ...
                <button
                    ...
                    className={`absolute right-0 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-gray-900 cursor-pointer ${
                        startIndex + 9 >= counties.length ? 'invisible' : ''
                        }`}
...
export default CountyCategories